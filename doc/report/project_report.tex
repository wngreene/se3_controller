...
\huge MIT 16.31 - Homework 1 \\ % The assignment title
\horrule{2pt} \\[0.5cm] % Thick bottom horizontal rule
}

\author{W. Nicholas Greene} % Your name

\date{\normalsize2014.09.22} % Today's date or a custom date
...
%% Given the described scene-camera geometry, and assuming the center of the image will be 
%% defined as pixel $(x_0, y_0)$ and the scale factor between world units and image units is 
%% $\alpha$, we can derive the transform from world to image coordinates as follows. 

%% Starting with the $x$ coordinate, we note that since the center of the camera lies in the 
%% $X=0$ plane and the image $x$-axis is aligned with the world $X$-axis, $x(X, Y, Z) = \alpha X + x_0$.
%% We see that the $X$ coordinate is scaled to pixel units via $\alpha$ and the addition of $x_0$
%% ensures that $X = 0$ is mapped to $x_0$ as desired.

%% To derive $y$ we may ignore the $X$ dimension and only consider the $YZ$-plane. When viewed
%% from this perspective we note that the image plane is tilted forward from vertical by $\theta$ 
%% radians such that the unit vector $(\cos \theta, -\sin \theta)$ lies parallel to the $y$-axis
%% in world coordinates. The $y$ coordinate of a point $(Y, Z)$ will be the projection onto 
%% this vector (suitably scaled and shifted). We see that 
%% $(\cos \theta, -\sin \theta) \cdot (Y, Z) = Y \cos \theta - Z \sin \theta$, which represents
%% the projection of the point $(Y, Z)$ onto the image plane. Finally we must scale the vector to
%% pixel units via $\alpha$ and add $y_0$ to ensure that the center pixel is as desired yielding
%% $y(X, Y, Z) = \alpha(Y \cos(\theta) - Z \sin(\theta)) + y_0$, completing the derivation.
...
%% To derive the linear constraints on $Z(x, y)$, we first note the projection equations:
%% \begin{align*}
%% x(X, Y, Z) &= \alpha X + x_0 \\
%% y(X, Y, Z) &= \alpha Y \cos(\theta) - \alpha Z \sin(\theta) + y_0.
%% \end{align*}

%% In our reconstruction task, we are given $(x, y)$ and must find $(X, Y, Z)$. We start by 
%% modifying the above equations to reflect our knowns and unknowns:
%% \begin{align*}
%% x &= \alpha X(x, y) + x_0 \\
%% y &= \alpha Y(x, y) \cos(\theta) - \alpha Z(x, y) \sin(\theta) + y_0.
%% \end{align*}
%% We see that we can solve for $X$ directly as $X(x, y) = \frac{x - x_0}{\alpha}$, but 
%% must introduce additional constraints for $Y$ and $Z$. 

%% For $Z$, we note that in the Simple World all vertical world-edges will have a constant $Z$ 
%% coodinate $c_z$ and appear as vertical image-edges. Thus, if we parameterize a vertical image-edge 
%% as $E_{v}(x, y) = (c_x, y)$, where $c_x$ is a constant, and plug this parameterzation into our 
%% equation for $Z$, we see that 
%% \begin{align*}
%% Z(x, y) &= Z(c_x, y) \\
%% &= Z(y) \\
%% &= \frac{Y(c_x, y)}{\tan(\theta)} - \frac{y}{\alpha \sin(\theta)} + \frac{y_0}{\alpha \sin(\theta)} \\
%% &= c_z.
%% \end{align*}
%% Taking derivatives, we have:
%% \begin{align*}
%% \frac{\partial Z}{\partial y} &= \frac{\partial Y}{\partial y} \frac{1}{\tan(\theta)} - \frac{1}{\alpha \sin(\theta} \\
%% &= \frac{1}{\alpha \cos(\theta)} \frac{1}{\tan(\theta)} - \frac{1}{\alpha \sin(\theta)} \\
%% & = 0,
%% \end{align*}
%% where we have used the fact that $\partial Y / \partial y = 1 / (\alpha \cos(\theta))$, as
%% described in the notes. Thus, we have 
%% \begin{align*}
%% \partial Z / \partial y = 0,
%% \end{align*}
%%  which we could have inferred
%% simply from $Z(y) = c_z$ (derivative of a constant is 0), but this machinery will help us
%% in the next section. Since the derivative operator is linear and can be represented by a
%% suitable matrix, this gives us our first linear constraint.

%% Now that we've encoded the constraints introduced by vertical world-edges, we now must 
%% encode the constraints for horizontal world-edges. In the Simple World, these are projected
%% to oblique image-edges with normal vector $\begin{bmatrix} n_x & n_y \end{bmatrix}^T$, 
%% which we can parameterize as
%% \begin{align*}
%% E_h(t) &= t \begin{bmatrix} v_1 \\ v_2 \end{bmatrix} + \begin{bmatrix} k_x \\ k_y \end{bmatrix}, 
%% \end{align*}
%% where $\begin{bmatrix} v_1 & v_2 \end{bmatrix}^T = \begin{bmatrix} -n_y & n_x \end{bmatrix}^T$
%% is the tangent vector to the edge and $k_x$ and $k_y$ are constants offsets. We plug this
%% parameterzation into our equation for $Z$:
%% \begin{align*}
%% Z(-t n_y + k_x, t n_x + k_y) &= \frac{Y(-t n_y + k_x, t n_x + k_y)}{\tan(\theta)} - 
%%      \frac{(t n_x + k_y)}{\alpha \sin(\theta)} + \frac{y_0}{\alpha \sin(\theta)} \\
%% &= \frac{\beta}{\tan(\theta)} - \frac{(t n_x + k_y)}{\alpha \sin(\theta)} + \frac{y_0}{\alpha \sin(\theta)},
%% \end{align*}
%% where we have used the fact that horizontal world-edges must have the same $Y$ coordinate $\beta$.

%% Taking the derivative with respect to $t$, we have $\partial Z / \partial t = -n_x / (\alpha \sin(\theta))$. 
%% Since the derivative of $Z$ with respect to $t$ is the same as the derivative of $Z$ in the
%% direction $\begin{bmatrix} -n_y & n_x \end{bmatrix}^T$, our linear constraint becomes:
%% \begin{align*}
%% \begin{bmatrix} -n_y & n_x \end{bmatrix}^T \begin{bmatrix} \partial Z / \partial x \\ \partial Z / \partial y \end{bmatrix}
%% &= \frac{-n_x}{\alpha \sin(\theta)}.
%% \end{align*}

%% Lastly, we must enforce the planarity constraints:
%% \begin{align*}
%% \partial^2 Z / \partial x^2 &= 0 \\
%% \partial^2 Z / \partial y^2 &= 0 \\
%% \partial^2 Z / \partial x \partial y &= 0,
%% \end{align*}
%% which ensure that surfaces are planar and do not curve in the $Z$ dimension.
...
%% Aij(:,:,c) = -ny*DH + nx*DV; %!!!! FILL MISSING KERNEL HERE;
%% \end{lstlisting}
...
%% Aij(:,:,c) = 0.1*[0 0 0; -1 2 -1; 0 0 0]'; %!!!! FILL MISSING KERNEL HERE;
%% \end{lstlisting}
...
%% Below are new views from \texttt{img1.jpg} and \texttt{img2.jpg}.
%% \begin{figure}[h!]
%%   \centering
%%   \begin{subfigure}[b]{0.45\textwidth}
%%     \includegraphics[width=\textwidth]{img1_new_view}
%%     \caption{New view from \texttt{img1.jpg}}
%%   \end{subfigure}
%%   \begin{subfigure}[b]{0.45\textwidth}
%%     \includegraphics[width=\textwidth]{img2_new_view}
%%     \caption{New view from \texttt{img2.jpg}}
%%   \end{subfigure}
%%   \caption{New views after reconstructing scene depths with Simple World assumption.}
%% \end{figure}
...
%% %----------------------------------------------------------------------------------------
%% \pagebreak
%% \section{Violating simple world assumptions}
%% Below are new views from \texttt{img3.jpg} and \texttt{img4.jpg} that demonstrate
%% reconstruction errors.
%% \begin{figure}[h!]
%%   \centering
%%   \begin{subfigure}[b]{0.45\textwidth}
%%     \includegraphics[width=\textwidth]{img3_new_view}
%%     \caption{New view from \texttt{img3.jpg}}
%%   \end{subfigure}
%%   \begin{subfigure}[b]{0.45\textwidth}
%%     \includegraphics[width=\textwidth]{img4_new_view}
%%     \caption{New view from \texttt{img4.jpg}}
%%   \end{subfigure}
%%   \caption{New views after reconstructing scene depths.}
%% \end{figure}

%% If we look at the edges computed from the edge-detection step we can see from where the 
%% problem stems.

%% \begin{figure}[h!]
%%   \centering
%%   \begin{subfigure}[b]{0.45\textwidth}
%%     \includegraphics[width=\textwidth]{img3_edges}
%%     \caption{Edges from \texttt{img3.jpg}}
%%   \end{subfigure}
%%   \begin{subfigure}[b]{0.45\textwidth}
%%     \includegraphics[width=\textwidth]{img4_edges}
%%     \caption{Edges from \texttt{img4.jpg}}
%%   \end{subfigure}
%%   \caption{Faulty edge detectors.}
%% \end{figure}
%% Here we can see that at least one edge from the green cube (right side in the input images) is not
%% detected using our simple, derivative based edge detector. This missing edge makes one face of the
%% cube bleed into the ground since the vertical occlusion edge (in \texttt{img3.jpg}) or the 
%% horizontal contact edge (in \texttt{img4.jpg}) constraints are no longer active.

%% %----------------------------------------------------------------------------------------
%% %	PROBLEM 6
%% %----------------------------------------------------------------------------------------
%% \section{Research problem - the real world (optional)}
